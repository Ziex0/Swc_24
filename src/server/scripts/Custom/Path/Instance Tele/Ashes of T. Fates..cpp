...
class Ashes_Fates : public CreatureScript
{
public:
	Ashes_Fates()
		: CreatureScript("Ashes_Fates") 
	{
	}
	
	struct Ashes_FatesAI : public ScriptedAI
	{
		Ashes_FatesAI(Creature *c) : ScriptedAI(c){}
    ...
    {
		return new Ashes_FatesAI(pCreature);
    }
	...
	{
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "|TInterface/ICONS/Thrown_1H_Harpoon_D_01Blue:35|t Talk to Lady Jaina ", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "|TInterface/ICONS/Thrown_1H_Harpoon_D_01Blue:35|t Talk to Thral ", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
		...
		switch (uiAction)
		{
			case GOSSIP_ACTION_INFO_DEF + 1: //Jaina
				pPlayer->CLOSE_GOSSIP_MENU();
				pPlayer->TeleportTo(534, 5036.2836f, -1768.5743f, 1325.1848f, 5.8211f);

				break;
			case GOSSIP_ACTION_INFO_DEF + 2: //Thrall
				pPlayer->CLOSE_GOSSIP_MENU();
				pPlayer->TeleportTo(534, 5503.8002f, -2740.2434f, 1488.6097f, 2.8491f);
